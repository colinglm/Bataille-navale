...
def coupintelligent(x, y, grillejoueur):
    if grillejoueur[x][y] in [21, 31, 41, 51]:  # Bateau touché 
        # Vérifier les cases adjacentes
        if y + 1 < len(grillejoueur[0]) and grillejoueur[x][y + 1] not in [21, 31, 41, 51]:
            return x, y + 1  # Tir à droite
        elif y - 1 >= 0 and grillejoueur[x][y - 1] not in [21, 31, 41, 51]:
            return x, y - 1  # Tir à gauche
        elif x + 1 < len(grillejoueur) and grillejoueur[x + 1][y] not in [21, 31, 41, 51]:
            return x + 1, y  # Tir en bas
        elif x - 1 >= 0 and grillejoueur[x - 1][y] not in [21, 31, 41, 51]:
            return x - 1, y  # Tir en haut
    else:
        # Tir aléatoire si aucun bateau n'est touché
        return coupordinateur()
...
def lancerjeu3():
    L = creer()  # Crée la grille du joueur
    L1 = creer()  # Crée la grille de l'ordinateur
    affich(L)
    placerbateaux(L)  # Place les bateaux du joueur
    placerordi(L1)  # Place les bateaux de l'ordinateur
    affichdouble(L1, L)  # Affiche les deux grilles
    ...
    while  partie_terminee!=True and  partie_termineeord!= True:
        # Le joueur joue
        i, j = coup()  # Le joueur choisit ses coordonnées manuellement
        
        # Effectuer le coup du joueur
        partie_terminee = jouer(L1, i, j, compteurs)
        affichdouble(L1, L)
        
        
        x,y=coupintelligent(x,y,L)
        # Effectuer le coup de l'ordinateur
        partie_termineeord = jouerordi(L, x, y, compteurso)
        affichdouble(L1, L)  # Affiche les grilles après le coup de l'ordinateur
        ...
        # Vérifie si tous les bateaux de l'ordinateur sont coulés
